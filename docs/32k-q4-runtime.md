...
后续把交接的 `K=1` 从“Router 返回数组的最后一项”修为“最后一行实际 Router
权重最高的一项”，因为前者按带偏置的 top-k 排序，并不等于最终聚合权重最大。专用
边界 profiler 显示 `A` / `你好` 的 40 个可学习层中，下一 token 平均分别复用
0.80 / 0.90 个这样的 top-1 尾部专家（保留全部六个则为 3.33 / 3.83 个），证明
局部性本身存在。不过 `Explain one plus one in one word.` 的 3-token 精确 runner
仍为关闭 0.76 t/s、weight-ranked K=1 为 0.77 t/s，trace `6111 2004 28` 完全相同。
收益仍处于噪声，服务默认继续关闭；这里的价值是排除了“选择错尾部专家”这个解释。
