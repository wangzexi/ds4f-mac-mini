...
也验证了一个更早的精确重叠设想：在每个 token 起点直接用 token ID 算出 L1 hash
路由的 six experts，并试图让它们在已完全驻留的 L0 运算期间读取。两组真实 server
运行各两次，输出 trace/响应哈希完全一致，但对照为 1.905 t/s、16.780 s Decode，提前
L1 读取反而为 1.700 t/s、18.836 s；两者仍是相同 1,192 loads / 18.92 GiB。说明该
机器上的早发读与 token 起始资源竞争，未转化为可用的隐藏时间。这条探针已撤回，正式
路径继续只在真实 router 选择完成后读取专家。

随后用完整的实际 expert-selection 轨迹检查替换策略。该夹具的 32-token Decode 中，
L1/L2 相邻 token 的 six-expert 集合平均仅重叠 0.19 / 0.23 个，分别触及 133 / 137
个不同专家；即使各自固定 128 槽，离线 LRU 也只有约 31 / 29% 命中，因此不能用小型
hash 专属缓存换来有效收益。全局离线 LRU 在 711 个可换槽下看似可达 62.4% 命中（当前
热度策略约 61.5%），但这忽略了 Prefill 进入 Decode 时的已驻留集合、in-flight
保护与 grouped load。真实 HTTP server 各两次 A/B 反而显示热度策略为 1.445 t/s、
22.612 s Decode（61.5%），纯全局 LRU 为 1.260 t/s、25.977 s（59.8%），后者多出
166 次 miss；token-ID trace 和最终响应哈希完全相同。故保持“累积路由权重 + 16-token
衰减 + LRU 同分决胜”的默认策略，纯 LRU 探针已撤回。
