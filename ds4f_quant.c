...
static float dot_q2_q8k(const q2_k_block *w,const q8_k_block *y,size_t n){float s=0;for(size_t b=0;b<n/256;++b){for(size_t k=0;k<256;++k){size_t group=k/16,l=k%16,base=32*(group/8)+16*(group&1),shift=((group/2)&3)*2;uint32_t q=(w[b].qs[base+l]>>shift)&3,sc=w[b].scales[group];s+=y[b].d*ds4f_f16_to_f32(w[b].d)*(float)(sc&15)*(float)q*(float)y[b].qs[k]-y[b].d*ds4f_f16_to_f32(w[b].dmin)*(float)(sc>>4)*(float)y[b].qs[k];}}return s;}
