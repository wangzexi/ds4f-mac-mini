...
static const char *tool_summary(ds4_help_tool tool) {
    switch (tool) {
    case DS4_HELP_DS4:
        return "Chat with a local DeepSeek model, run one-shot prompts, or inspect the fixed runtime.";
    case DS4_HELP_SERVER:
        return "Serve the loaded DeepSeek model through OpenAI, Responses, Anthropic, and completion-compatible HTTP APIs.";
    case DS4_HELP_AGENT:
...
static void print_server_api(FILE *fp, const help_colors *c) {
    ...
    para(fp, c, "This fixed runtime exposes only the deepseek-v4-flash model ID.");
    fputc('\n', fp);
}
