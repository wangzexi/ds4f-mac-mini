...
static void usage(const char *program) {
    fprintf(stderr,
            ...
            "Reads one prompt per stdin line. Each request gets a fresh context while the selected SSD expert cache stays resident. "
            "Defaults to 128K context; set DS4F_FAST_CONTEXT_K=8, 16, 24, 32, 128, or 256.\n"
            "At contexts up to 32K, the default is a direct 440-expert cache; setting DS4F_FAST_CACHE_GIB keeps the legacy total-budget mode.\n",
            program);
}
...
int main(int argc, char **argv) {
    ...
    if (context_tokens_from_env(&context_size)) {
        fprintf(stderr, "ds4f-reuse: DS4F_FAST_CONTEXT_K must be 8, 16, 24, 32, 128, or 256\n");
        return 2;
    }
