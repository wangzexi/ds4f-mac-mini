...
static const char *db_weight_file(const st_db *db, const char *name) {
    int wi = hmap_get(&db->weight_map, name);
    if (wi < 0) {
        fprintf(stderr, "error: HF tensor not found in index: %s\n", name);
        exit(1);
    }
    return db->weights[wi].file;
}
...
static void write_full_gguf(st_db *db, const gguf_file *tmpl, const output_context *out_ctx,
                            ...
                            const imatrix_store *imatrix,
                            bool preserve_routed_template) {
    FILE *fp = fopen(out_path, "wb");
    if (!fp) die_errno("open output", out_path);
    FILE *template_fp = NULL;
    if (preserve_routed_template) {
        template_fp = fopen(tmpl->path, "rb");
        if (!template_fp) die_errno("open GGUF template", tmpl->path);
    }
    ...
    for (uint64_t i = 0; i < out_ctx->n_tensors; i++) {
        ...
        if (preserve_routed_template && parse_expert_tensor(dst->name).is_expert) {
            if (src->type != dst->type || src->size != dst->size) {
                fprintf(stderr,
                        "error: preserved routed tensor changed type or size: %s\n",
                        dst->name);
                exit(1);
            }
            if (fseeko(template_fp,
                       ...
                       SEEK_SET) != 0) {
                die_errno("seek GGUF template", tmpl->path);
            }
            ...
            while (remaining) {
                ...
                if (fread(buffer, 1, chunk, template_fp) != chunk) {
                    die_errno("read GGUF template tensor", tmpl->path);
                }
            ...
            free(buffer);
            const size_t padded = ds4q_pad(src->size, out_ctx->alignment);
            write_padding(fp, padded - src->size);
            fprintf(stderr, "       copied %.2f MiB byte-exact from template\n",
                    (double)src->size / 1048576.0);
            continue;
        }
    ...
    if (template_fp) fclose(template_fp);
    fclose(fp);
}

static void stream_add_file(const st_db *db, const char *hf_name,
                            const char ***files, int *n_files, int *cap_files) {
    const char *file = db_weight_file(db, hf_name);
    for (int i = 0; i < *n_files; i++) {
        if (strcmp((*files)[i], file) == 0) return;
    }
    if (*n_files == *cap_files) {
        *cap_files = *cap_files ? *cap_files * 2 : 8;
        *files = xrealloc((void *)*files, (size_t)*cap_files * sizeof((*files)[0]));
    }
    (*files)[(*n_files)++] = file;
}

static void stream_add_regular_files(const st_db *db, const char *gguf_name,
                                     const char ***files, int *n_files, int *cap_files) {
    char *hf_name = hf_name_for_regular(gguf_name);
    stream_add_file(db, hf_name, files, n_files, cap_files);
    if (str_ends(hf_name, ".weight")) {
        char *scale_name = xstrdup(hf_name);
        strcpy(scale_name + strlen(scale_name) - strlen(".weight"), ".scale");
        if (db_has(db, scale_name)) stream_add_file(db, scale_name, files, n_files, cap_files);
        free(scale_name);
    }
    free(hf_name);
}

static void stream_add_expert_files(const st_db *db, const expert_tensor *expert, int n_experts,
                                    const char ***files, int *n_files, int *cap_files) {
    const char *wid = expert_part_name(expert->part);
    for (int xid = 0; xid < n_experts; xid++) {
        char prefix[256];
        if (expert->scope == EXP_SCOPE_MTP) {
            snprintf(prefix, sizeof(prefix), "mtp.%d.ffn.experts.%d.%s", expert->layer, xid, wid);
        } else {
            snprintf(prefix, sizeof(prefix), "layers.%d.ffn.experts.%d.%s", expert->layer, xid, wid);
        }
        char weight_name[320];
        char scale_name[320];
        snprintf(weight_name, sizeof(weight_name), "%s.weight", prefix);
        snprintf(scale_name, sizeof(scale_name), "%s.scale", prefix);
        stream_add_file(db, weight_name, files, n_files, cap_files);
        if (db_has(db, scale_name)) stream_add_file(db, scale_name, files, n_files, cap_files);
    }
}

static void print_stream_plan(st_db *db, const gguf_file *tmpl, int n_experts) {
    printf("stream_plan_version: 1\n");
    for (uint64_t i = 0; i < tmpl->n_tensors; i++) {
        ...
        if (expert.is_expert) {
            stream_add_expert_files(db, &expert, n_experts, &files, &n_files, &cap_files);
        } else {
            stream_add_regular_files(db, tmpl->tensors[i].name, &files, &n_files, &cap_files);
        }
        printf("tensor\t%" PRIu64 "\t%s\t", i, tmpl->tensors[i].name);
        for (int j = 0; j < n_files; j++) {
            if (j) putchar(',');
            fputs(files[j], stdout);
        }
        putchar('\n');
        free((void *)files);
    }
...
static bool *parse_tensor_list(const char *raw, uint64_t n_tensors) {
    bool *selected = xcalloc((size_t)n_tensors, sizeof(selected[0]));
    char *copy = xstrdup(raw);
    char *cursor = copy;
    while (cursor && *cursor) {
        char *comma = strchr(cursor, ',');
        if (comma) *comma = '\0';
        if (!cursor[0]) die("empty tensor index in --tensor-list");
        char *end = NULL;
        errno = 0;
        unsigned long long index = strtoull(cursor, &end, 10);
        if (errno || end == cursor || *end || index >= n_tensors) {
            fprintf(stderr, "error: bad tensor index in --tensor-list: %s\n", cursor);
            exit(1);
        }
        selected[index] = true;
...
static void write_stream_header(const gguf_file *tmpl, const output_context *out_ctx,
                                const char *out_path, const imatrix_store *imatrix) {
    FILE *fp = fopen(out_path, "wb");
    if (!fp) die_errno("open stream output", out_path);
    if (fwrite("GGUF", 1, 4, fp) != 4) die("write GGUF magic failed");
    write_u32(fp, tmpl->version);
    write_u64(fp, tmpl->n_tensors);
    write_u64(fp, tmpl->n_kv + out_ctx->n_kv_extra);
    ...
    if ((size_t)pos > out_ctx->data_offset) die("GGUF metadata larger than planned");
    write_padding(fp, out_ctx->data_offset - (size_t)pos);
    const uint64_t total_size = (uint64_t)out_ctx->data_offset + (uint64_t)out_ctx->tensor_bytes;
    if (total_size > (uint64_t)INT64_MAX || ftruncate(fileno(fp), (off_t)total_size) != 0) {
        die_errno("preallocate stream output", out_path);
    }
    if (fflush(fp) != 0 || fsync(fileno(fp)) != 0) die_errno("sync stream output", out_path);
    fclose(fp);
}

static void write_stream_tensors(st_db *db, const gguf_file *tmpl, const output_context *out_ctx,
                                 const char *out_path, int n_experts, int n_threads,
                                 const imatrix_store *imatrix, const bool *selected) {
    FILE *fp = fopen(out_path, "r+b");
    if (!fp) die_errno("open stream output", out_path);
    uint64_t written = 0;
    for (uint64_t i = 0; i < out_ctx->n_tensors; i++) {
        ...
        const tensor_meta *src = &tmpl->tensors[i];
        const tensor_meta *dst = &out_ctx->tensors[i];
        fprintf(stderr, "stream tensor [%" PRIu64 "/%" PRIu64 "] %s -> %s\n",
                i + 1, out_ctx->n_tensors, dst->name, ds4q_type_name(dst->type));
        if (fseeko(fp, (off_t)((uint64_t)out_ctx->data_offset + dst->new_offset), SEEK_SET) != 0) {
            die_errno("seek stream output", out_path);
        }
        byte_buf data = generate_tensor(db, dst->name, src, dst->type, n_experts, n_threads, imatrix);
        ...
        if (fwrite(data.data, 1, data.size, fp) != data.size) die_errno("write stream tensor", out_path);
        write_padding(fp, ds4q_pad(data.size, out_ctx->alignment) - data.size);
        free(data.data);
        written++;
    }
    if (written == 0) die("--tensor-list selected no tensors");
    if (fflush(fp) != 0 || fsync(fileno(fp)) != 0) die_errno("sync stream tensors", out_path);
    fclose(fp);
    fprintf(stderr, "stream tensors complete: %" PRIu64 "\n", written);
}
...
static void usage(const char *argv0) {
    ...
    printf("  --stream-plan          print HF shard dependencies for each output tensor\n");
    printf("  --stream-init          create a preallocated output for range writes\n");
    printf("  --tensor-list CSV      generate only the listed output tensor indexes into an initialized output\n");
    ...
    printf("  --preserve-routed-template copy routed expert bytes exactly from the template\n");
...
static params parse_args(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(arg, "-h") == 0 || strcmp(arg, "--help") == 0) {
            ...
            p.dry_run = true;
        } else if (strcmp(arg, "--stream-plan") == 0) {
            p.stream_plan = true;
        } else if (strcmp(arg, "--stream-init") == 0) {
            p.stream_init = true;
        } else if (strcmp(arg, "--tensor-list") == 0) {
            p.tensor_list = need_value(argc, argv, &i, arg);
        ...
        } else if (strcmp(arg, "--preserve-routed-template") == 0) {
            p.preserve_routed_template = true;
    ...
    if (p.stream_plan && (p.stream_init || p.tensor_list)) {
        die("--stream-plan cannot be combined with --stream-init or --tensor-list");
    }
    if (p.stream_init && p.preserve_routed_template) {
        die("--stream-init cannot be combined with --preserve-routed-template");
    }
    if (p.preserve_routed_template && p.imatrix_file) {
        die("--preserve-routed-template cannot be combined with --imatrix");
    }
    if (!p.dry_run && !p.compare_tensor && !p.stream_plan && !p.out_gguf) die("--out is required unless --dry-run or --compare-tensor is used");
    if (p.tensor_list && !p.out_gguf) die("--tensor-list requires --out");
    if (p.compare_tensor && !p.compare_gguf) p.compare_gguf = p.template_gguf;
    if (p.out_gguf && file_exists(p.out_gguf) && !p.overwrite && (!p.tensor_list || p.stream_init)) {
        die("output exists; use --overwrite");
    }
    return p;
}
...
int main(int argc, char **argv) {
    ...
    if (p.preserve_routed_template) {
        for (uint64_t i = 0; i < out_ctx.n_tensors; i++) {
            if (!parse_expert_tensor(tmpl.tensors[i].name).is_expert) continue;
            if (tmpl.tensors[i].type != out_ctx.tensors[i].type ||
                tmpl.tensors[i].size != out_ctx.tensors[i].size) {
                fprintf(stderr,
                        "error: --preserve-routed-template conflicts with routed override for %s\n",
                        tmpl.tensors[i].name);
                exit(1);
            }
        }
    ...
    if (p.stream_plan) {
        st_db db;
        db_open(&db, p.hf_dir);
        print_stream_plan(&db, &tmpl, p.n_experts);
        db_close(&db);
        imatrix_free(&imatrix);
        free_gguf_file(&tmpl);
    ...
    if (p.stream_init) {
        write_stream_header(&tmpl, &out_ctx, p.out_gguf, &imatrix);
        fprintf(stderr, "initialized stream output %s (%zu bytes)\n",
                p.out_gguf, out_ctx.data_offset + out_ctx.tensor_bytes);
        if (!p.tensor_list) {
            imatrix_free(&imatrix);
            free_gguf_file(&tmpl);
            free(out_ctx.tensors);
    ...
    if (p.tensor_list) {
        bool *selected = parse_tensor_list(p.tensor_list, out_ctx.n_tensors);
        write_stream_tensors(&db, &tmpl, &out_ctx, p.out_gguf, p.n_experts, p.n_threads,
                             &imatrix, selected);
        free(selected);
    } else {
        write_full_gguf(&db, &tmpl, &out_ctx, p.out_gguf, p.n_experts, p.n_threads,
                        &imatrix, p.preserve_routed_template);
        fprintf(stderr, "wrote %s\n", p.out_gguf);
    }
