 ...
 * Prefill; `...=reuse-heat` applies it only after a second cache hit; and
 * `...=token-lru` compares recency by Decode-token epoch.  The fixed Mini's
 * `adaptive` policy uses the exact uncached Prefill row count.
