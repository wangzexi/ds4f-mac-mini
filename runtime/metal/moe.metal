...
/* Exact slots6 counterpart of the address-table split kernel.  The six
 * direct buffer bindings are the established numerical baseline; active_mask
 * merely leaves inactive pair-output slots untouched for the later stage. */
kernel void kernel_mul_mv_slots6_iq2_xxs_pair_swiglu_masked_f32(
        constant ds4_metal_args_mul_mv_id & args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args & act,
    ...
    const int iid1 = tgpig.z / args.nei0;
    const int idx  = tgpig.z % args.nei0;
    if ((split.active_mask & (1u << (uint)idx)) == 0) return;
    ...
    device const char *src1_cur = src1 + i11 * args.nb11 + i12 * args.nb12;
    device char *dst_gate_cur = dst_gate +
        (idx * args.ne0 + i12 * args.ne1 * args.ne0) * sizeof(float);
    device char *dst_up_cur = dst_up +
        (idx * args.ne0 + i12 * args.ne1 * args.ne0) * sizeof(float);
    ds4_metal_args_mul_mv args0 = {
        args.ne00, args.ne01, 1,
        args.nb00, args.nb01, args.nb02, args.nb02,
    ...
    kernel_mul_mv_iq2_xxs_pair_f32_impl<N_R0_IQ2_XXS>(
        args0, src0_gate_cur, src0_up_cur, src1_cur, dst_gate_cur,
        dst_up_cur, shmem, tgpig, tiisg, sgitg);
    ...
    const float route_weight = route_w[0];
    if (tiisg == 0) {
        for (int row = 0; row < N_R0_IQ2_XXS && first_row + row < args.ne0;
             ++row) {
            const uint out_row = first_row + row;
            float g = gate_f32[out_row];
            float u = up_f32[out_row];
